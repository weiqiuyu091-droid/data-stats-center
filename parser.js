...
function stripHK(s){ return s.replace(/^(?:香港|港|香)s*[:：]?s*/i,"").replace(/s*(?:香港|港|香)s*$/i,"").replace(/^[：:,，s]+/,"").trim(); }
...
function norm(s, debug){
  ...
  t = t.replace(/[+＋]/g,'').replace(/。/g,'').replace(/^(.+)香港澳门$/g, '澳$1；港$1').replace(/(\d+(?:\.\d+)?\s*(?:斤|米|块|元|文))\s*[，]\s*(?=\d)/g, '$1；')
    ...
    .replace(/(?:新澳|澳门|澳)$/g,'')
    // 总额提示剥离(行尾): "每数50总数350"→"每数50"
    .replace(/总数\d+(?:\.\d+)?\s*(?:块|斤|米)?\s*$/g,'')
    ...
    .replace(/号个/g,'号各').replace(/一个号各/g,'各').replace(/一个号/g,'').replace(/号各/g,'各数').replace(/名数/g,'各数').replace(/每个号码/g,'各数').replace(/每个号/g,'各数').replace(/每数/g,'各数').replace(/号\/(\d)/g,'号$1').replace(/个组/g,'各组').replace(/个\//g,'各').replace(/一个\s*$/g,'')
    ...
    .replace(/平特(三连|二连|四连|五连)(?!肖)/g, '$1')
    // "虎狗平特各20"/"虎平特20" → "平特虎狗各20" (平特在生肖后时前置)
    .replace(new RegExp(`([${ZODIAC_CHARS}]{1,})\\s*平特\\s*(?:各\\s*)?(\\d+(?:\\.\\d+)?)\\s*(斤|米|块)?`, 'g'), '平特$1各$2$3')
    .replace(/(\d)[Oo](\d)/g,'$10$2').replace(/(\d+)[A-Za-z]+/g,'$1')
    .replace(/门\s*$/g,'')
    .replace(/(\d{1,2})\.+(?=\d|$)/g,'$1 ').replace(/(\d{1,2})\.(?![\d]{1,2})/g,'$1 ')
