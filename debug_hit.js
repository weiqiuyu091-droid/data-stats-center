...
// Build a wrapper that runs everything inside vm context
const wrapperCode = `
function makeEl() { return { textContent: '', style: {}, innerHTML: '', appendChild: function(){}, querySelectorAll: function(){ return []; }, children: [], classList: { add: function(){}, remove: function(){} }, addEventListener: function(){}, setAttribute: function(){}, remove: function(){}, getAttribute: function(){ return null; } }; }
var els = {
  sumTotalBet: makeEl(), sumTotalPayout: makeEl(), sumNetProfit: makeEl(), sumNetAfterWater: makeEl(),
  sumHitCount: makeEl(), sumWinAmount: makeEl(), waterInfo: makeEl(),
  waveRedBet: makeEl(), waveGreenBet: makeEl(), waveBlueBet: makeEl(),
  waveRedHit: makeEl(), waveGreenHit: makeEl(), waveBlueHit: makeEl(),
  waveStatRow: makeEl(), settlementBody_tbody: makeEl(),
  footBet: makeEl(), footPayout: makeEl(), footNet: makeEl(),
  inputRule: { value: '', addEventListener: function(){}, removeEventListener: function(){} },
  numGrid: makeEl(), zodiacGrid: makeEl()
};
var document = {
  getElementById: function(id) { return els[id] || makeEl(); },
...
// Set up lottery results AFTER code runs (let declarations are in the code)
liveResult = __liveResult;
hkResult = __hkResult;

// Run analyze
document.getElementById('inputRule').value = testData_;
analyze();

// Run calcAll
calcAll();

// Now compute detailed results
var macauWinRows = [];
var macauHitTotal = 0;
betSummary.forEach(function(r, i) {
  var sr = settleRow(r, __macauNums, __macauTeMa, __macauFlatZodiacs);
  if(sr.note) {
    macauHitTotal += sr.hitBet;
    macauWinRows.push({idx:i, bet:r.bet, hitBet:sr.hitBet, display:r.display, type:r.type, note:sr.note});
  }
});
...
hkBetSummary.forEach(function(r, i) {
  var sr = settleRow(r, __hkNums, __hkTeMa, __hkFlatZodiacs);
  if(sr.note) {
    hkHitTotal += sr.hitBet;
    hkWinRows.push({idx:i, bet:r.bet, hitBet:sr.hitBet, display:r.display, type:r.type, note:sr.note});
  }
});

console.log('=== Macau Winning Rows ===');
macauWinRows.forEach(function(r) {
  console.log('M[' + r.idx + '] hitBet=' + r.hitBet + ' bet=' + r.bet + ' type=' + r.type + ' display=' + JSON.stringify(r.display) + ' note=' + JSON.stringify(r.note));
});
console.log('Macau total hitBet: ' + Math.round(macauHitTotal * 100) / 100);

console.log('');
console.log('=== HK Winning Rows ===');
hkWinRows.forEach(function(r) {
  console.log('H[' + r.idx + '] hitBet=' + r.hitBet + ' bet=' + r.bet + ' type=' + r.type + ' display=' + JSON.stringify(r.display) + ' note=' + JSON.stringify(r.note));
});
console.log('HK total hitBet: ' + Math.round(hkHitTotal * 100) / 100);

console.log('');
console.log('COMBINED: ' + Math.round((macauHitTotal + hkHitTotal) * 100) / 100);
console.log('Macau total msgBet: ' + Math.round(betSummary.reduce(function(a,r){return a+r.bet;},0) * 100) / 100);
console.log('HK total msgBet: ' + Math.round(hkBetSummary.reduce(function(a,r){return a+r.bet;},0) * 100) / 100);
`;

const sandbox = {
  console: console,
  testData_: testData,
  __liveResult: { numbers: macauNums, teMa: macauTeMa, flatZodiacs: macauFlatZodiacs, validCount: 7 },
  __hkResult: { numbers: hkNums, teMa: hkTeMa, flatZodiacs: hkFlatZodiacs },
  __macauNums: macauNums,
  __macauTeMa: macauTeMa,
  __macauFlatZodiacs: macauFlatZodiacs,
  __hkNums: hkNums,
  __hkTeMa: hkTeMa,
  __hkFlatZodiacs: hkFlatZodiacs
};

vm.createContext(sandbox);
try {
  vm.runInContext(wrapperCode, sandbox, { timeout: 30000 });
} catch(e) {
